...
public:
	...
	// TODO: Implement this method
	// The fist policy added should have the history written to it.
...
private:
	...
	std::unique_ptr<Policy> *myPolicies[2];
	int myNum = 0;
...
#define LOG_LVL0(messageStream)                                     \
	do                                                              \
	{                                                               \
		std::ostringstream stream;                                  \
		stream << "<level 0> " << messageStream;                    \
		Logger::instance().write(__FILE__, __LINE__, stream.str()); \
	} while (false)
#else
#define LOG_LVL0(...) \
	do                \
...
#define LOG_LVL1(messageStream)                                     \
	do                                                              \
	{                                                               \
		std::ostringstream stream;                                  \
		stream << "<level 1> " << messageStream;                    \
		Logger::instance().write(__FILE__, __LINE__, stream.str()); \
	} while (false)
#else
#define LOG_LVL1(...) \
...
#define LOG_LVL2(messageStream)                                     \
	do                                                              \
	{                                                               \
		std::ostringstream stream;                                  \
		stream << "<level 2> " << messageStream;                    \
		Logger::instance().write(__FILE__, __LINE__, stream.str()); \
	} while (false)

/// \brief Report an error message.
#define LOG_ERROR(messageStream)                                    \
	do                                                              \
	{                                                               \
		std::ostringstream stream;                                  \
		stream << "<error>   " << messageStream;                    \
		Logger::instance().write(__FILE__, __LINE__, stream.str()); \
	} while (false)
